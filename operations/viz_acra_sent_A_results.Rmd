---
title: "ACRA Sentiment - Results - Attacker A"
output:
  html_document:
    df_print: paged
date: '2022-10-10'
...
```{r warning=F}
dname <- "results/sentiment_A/exp_classifiers/"
...
```{r}
dname <- "results/sentiment_A/exp_classifiers_noCK_5/"
no_CK <- get_data(dname)

result_noCK <- no_CK %>% filter(flag != "svm") %>%   mutate (Classifier = flag) %>% group_by(Classifier) %>% 
  summarise(`Mean Accuracy Clean Data` = mean(acc_raw_clean),
            `Std. Dev. Accuracy Clean Data` = sd(acc_raw_clean)/sqrt(10),
            `Mean Accuracy Tainted Data` = mean(acc_raw_att),
            `Std. Dev. Accuracy Tainted Data` = sd(acc_raw_att)/sqrt(10),
